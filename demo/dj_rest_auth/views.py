...
class OTPLoginView(GenericAPIView):
    permission_classes = (AllowAny,)
    serializer_class = api_settings.LOGIN_SERIALIZER
    # serializer_class = api_settings.OTP_LOGIN_SERIALIZER
    ...
    def post(self, request, *args, **kwargs):
        ...
        request_data = {'username':mutable_data['username'],
                'password':mutable_data['password']}
        
        try:
            u = User.objects.get(username=mutable_data['username'])
        except:
            return Response(status=404, msg="Record Not found", data='')
        if(u.is_active==False):
            return Response(msg="User Is Not Activated", status=404, data='')
        
        try:            
            # serializer = self.serializer_class(data=data)
            serializer = OTPLoginSerializer(data=request_data)
            serializer.is_valid(raise_exception=True)
        except Exception as e:         
            print(e)   
            return Response(status=404, msg="Not A Valid User", data='')
        
        if 'code' not in mutable_data:
            data = serializer.data.copy()
            code = random_generator()
            print("this is code",code)
            current_time = datetime.datetime.utcnow().strftime('%Y-%m-%dT%H:%M:%S')
            check_user_exist = OtpLogin(code=code, user_id=data['user_id'],created_date = current_time)
            check_user_exist.save()            
            return Response(data={'error':'0','msg':'Your code has been generated','action':'Code','code':code}, status=status.HTTP_200_OK)
        else:
            verifycode = mutable_data['code']
            try:
                user_vcode = OtpLogin.objects.get(code=verifycode)
            except:
                return Response(status=404, data={"error":1,"msg":"not a valid code"}) #msg="Not a Valid code"
            data = serializer.data.copy() 
            user_id_verify = user_vcode.user_id
            code_time = user_vcode.created_date.replace(tzinfo=None)
            current_time = datetime.datetime.utcnow()
            code_generated_time = current_time - code_time
            if code_generated_time.seconds > 300:
                user_vcode.delete()
                return Response(data={"error":1,"msg":"your code has been expired"}, status=404)
            user_id = data['user_id']
            if(int(user_id) != int(user_id_verify) ):
                return Response(data='', msg="User Not Exist", status=404)
            data = {"user":data,'error':"0",'msg':"Login Successfully"}
            user_vcode.delete()
            self.serializer = self.get_serializer(data=self.request.data)
            self.serializer.is_valid(raise_exception=True)
            self.login()
